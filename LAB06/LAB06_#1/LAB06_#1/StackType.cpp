...
void StackType::ReplaceItem(ItemType oldItem, ItemType newItem) {
    //함수 설명: 링크 구조로 연결되어 있는 노드들을 top 에서부터 시작하여 하나씩 검사해 가며 oldItem 값과 같은 아이템을 newItem으 로 변경해 준다
    NodeType *location = topPtr; //현재 노드를 가리기기 위한 포인터 선언 및 초기화
    while (location != NULL) {
