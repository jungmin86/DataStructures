...
void StackType::Copy(StackType& anotherStack) {
    ...
    if (anotherStack.topPtr == NULL) topPtr = NULL;
        ...
        std::cout << "a.topPtr1: " << anotherStack.topPtr << std::endl; //각 요소가 다른 메모리에 저장되어야 딥 카피
        std::cout << "topPtr2: " << topPtr << std::endl;
...
void StackType::operator= (StackType anotherStack) { //멤버 함수로만 구현 가능
    NodeType* ptr1;
    NodeType* ptr2;
    if (anotherStack.topPtr == NULL) topPtr = NULL;
    else {
